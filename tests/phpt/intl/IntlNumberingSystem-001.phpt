--TEST--
Intl\NumberingSystem has expected values
--EXTENSIONS--
ecma_intl
--FILE--
<?php
...
foreach ($tests as $test) {
    assert(
        $test['numberingSystem']->value === $test['expectedValue'],
        sprintf('Actual "%s" is not the same as expected "%s"', $test['numberingSystem']->value, $test['expectedValue']),
    );
    echo json_encode($test['numberingSystem']) . "\n";
}
?>
--EXPECT--
"adlm"
